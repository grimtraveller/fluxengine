...
void Camera::lookAt(const Vector3 &eye, const Vector3 &target, const Vector3 &up)
{
    ...
    m_viewMatrix._m[0][0] = m_xAxis.x;
    m_viewMatrix._m[1][0] = m_xAxis.y;
    m_viewMatrix._m[2][0] = m_xAxis.z;
    m_viewMatrix._m[3][0] = -Vector3::dot(m_xAxis, eye);

    m_viewMatrix._m[0][1] = m_yAxis.x;
    m_viewMatrix._m[1][1] = m_yAxis.y;
    m_viewMatrix._m[2][1] = m_yAxis.z;
    m_viewMatrix._m[3][1] = -Vector3::dot(m_yAxis, eye);

    m_viewMatrix._m[0][2] = m_zAxis.x;
    m_viewMatrix._m[1][2] = m_zAxis.y;
    m_viewMatrix._m[2][2] = m_zAxis.z;    
    m_viewMatrix._m[3][2] = -Vector3::dot(m_zAxis, eye);

    // Extract the pitch angle from the view matrix.
    m_accumPitchDegrees = Math::radiansToDegrees(asinf(m_viewMatrix._m[1][2]));
...
void Camera::perspective(float fovx, float aspect, float znear, float zfar)
{
    ...
    m_projMatrix._m[0][0] = xScale;
    m_projMatrix._m[0][1] = 0.0f;
    m_projMatrix._m[0][2] = 0.0f;
    m_projMatrix._m[0][3] = 0.0f;

    m_projMatrix._m[1][0] = 0.0f;
    m_projMatrix._m[1][1] = yScale;
    m_projMatrix._m[1][2] = 0.0f;
    m_projMatrix._m[1][3] = 0.0f;

    m_projMatrix._m[2][0] = 0.0f;
    m_projMatrix._m[2][1] = 0.0f;
    m_projMatrix._m[2][2] = (zfar + znear) / (znear - zfar);
    m_projMatrix._m[2][3] = -1.0f;

    m_projMatrix._m[3][0] = 0.0f;
    m_projMatrix._m[3][1] = 0.0f;
    m_projMatrix._m[3][2] = (2.0f * zfar * znear) / (znear - zfar);
    m_projMatrix._m[3][3] = 0.0f;
...
void Camera::updatePosition(const Vector3 &direction, float elapsedTimeSec)
{
    ...
    if (m_currentVelocity.magnitude_sq() != 0.0f)
    {
...
{
    Matrix4x4 m = newOrientation.toMatrix4x4();
    ...
    m_accumPitchDegrees = Math::radiansToDegrees(asinf(m._m[1][2]));
...
void Camera::updateViewMatrix()
{
    ...
    m_viewMatrix = m_orientation.toMatrix4x4();

    m_xAxis.set(m_viewMatrix._m[0][0], m_viewMatrix._m[1][0], m_viewMatrix._m[2][0]);
    m_yAxis.set(m_viewMatrix._m[0][1], m_viewMatrix._m[1][1], m_viewMatrix._m[2][1]);
    m_zAxis.set(m_viewMatrix._m[0][2], m_viewMatrix._m[1][2], m_viewMatrix._m[2][2]);
    m_viewDir = -m_zAxis;
    ...
    m_viewMatrix._m[3][0] = -Vector3::dot(m_xAxis, m_eye);
    m_viewMatrix._m[3][1] = -Vector3::dot(m_yAxis, m_eye);
    m_viewMatrix._m[3][2] = -Vector3::dot(m_zAxis, m_eye);
}